...
READLINK='/usr/local/Cellar/coreutils/8.25/bin/greadlink -e '
...
find_git_repo() {
    local dir=$( /usr/local/Cellar/coreutils/8.25/bin/greadlink -e . )
    until [ $dir -ef '/' ]; do
        ...
        dir=$( /usr/local/Cellar/coreutils/8.25/bin/greadlink -e $dir/.. )
    done
    GIT_REPO=''
    return
}
...
update_tmux() {
    ...
    CMD="/usr/local/Cellar/coreutils/8.25/bin/greadlink -e $(pwd)"
    CWD=`eval ${CMD}`
